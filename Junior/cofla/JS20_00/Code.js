...
class Datos {
	...
	alumno(cantidad){
		...
		for (let i = 0; i < cantidad; i++) {
			...
			while (num == 32 && nombres.includes("cofla")) {
...
const aritmeticaI = materias(`aritmeticaI`, datos.profe(1), datos.alumno(numAleatorio.tenToThrirty()));
const aritmeticaII = materias(`aritmeticaII`, datos.profe(1), datos.alumno(numAleatorio.tenToThrirty()));
...
function print() {
	clases.forEach((internalClases) =>{
		...
		arrayPrint1 = [];
		for (let i = 0; i < 15; i++) {
			arrayPrint0[i] = internalClases.alumnos.shift();
			while (arrayPrint0.includes(undefined)) {
				arrayPrint0.pop();
			}
		}
		for (let i = 0; i < 15; i++) {
			arrayPrint1[i] = internalClases.alumnos.shift();
